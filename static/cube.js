...
class Controls {
    ...
    initDraggable() {
        ...
        this.draggable.onDragEnd = position => {
            ...
            if (this.scramble !== null && !UsingHandgesture) return;
...
class Video extends Animation {
    ...
    activate(){
                navigator.mediaDevices.getUserMedia({video: true})
                    .then(stream=>{
                        this.usercam.srcObject = stream;
                        this.camstream = stream;
                        this.socket = io("ws://localhost:5000") //Flask server
                        gsap.to(".usercam",{
                            scaleY:1,
                            duration:0.35
                        })
                        this.initSocketListen();
                        UsingHandgesture = true
                    })
    ...
    sendFrame(){
        if(this.socket){
            if(this.socket.connected){
                this.canvas.width = this.usercam.videoWidth;
                this.canvas.height = this.usercam.videoHeight;
                this.ctx.drawImage(this.usercam, 0, 0, this.canvas.width, this.canvas.height);

                const frameData = this.canvas.toDataURL('image/jpeg');
                const timestamp = Date.now();

                const payload = JSON.stringify({ image: frameData, timestamp: timestamp });
    ...
    commandCube(data){
        ...
        let command = null
        if(data.side=="Right"){
            if(data.command=="FaceFront"){
                command = "F";
            }else if(data.command="HorizontalTop"){
                command = "U";
            }else if(data.command="VerticalOuterSide"){
                command = "R";
            }else if(data.command="FaceMid"){
            }else if(data.command="HorizontalMid"){
            }else if(data.command="VerticalMid"){
            }else if(data.command="FaceBack"){
                command = "B";
            }else if(data.command = "VerticalInnerSide"){
                command = "L'"
            }else if(data.command = "HorizontalBot"){
                command = "D"
            }
        ...
        let converted = this.scrambler.convertMove(command); //this.scrambler doesnt exist?
        let move = converted;
        let layer = this.controls.getLayer(move.position);
        this.controls.flipAxis = new THREE.Vector3();
